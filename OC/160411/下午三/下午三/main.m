...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        //谓词 是指一个类 NSPredicate,主要用于筛选和查找， 谓词是基于KVC的
        
        NSArray *arr=@[@78,@58,@47,@89,@67];
        //创建谓词
        NSPredicate *pre1=[NSPredicate predicateWithFormat:@"SELF>60"];
        //根据谓词进行筛选
        NSLog(@"%@",[arr filteredArrayUsingPredicate:pre1]);
        ...
        NSLog(@"pre3=%@",[arr1 filteredArrayUsingPredicate:pre3]);
        //ALL表示所有
        ...
        // {}里面有的数据才会显示IN  范围：BETWEEN都是可以的1,IN精确查找，BETWEEN模糊查找
        ...
        //是否用XXX开头BEGINSWITH
        NSPredicate *pre6=[NSPredicate predicateWithFormat:@"name BEGINSWITH[c] 'yang'"];//默认为d区分大小写，c不区分
        ...
        NSPredicate *pre10=[NSPredicate predicateWithFormat:@"SELF MATCHES%@",regularExpress];
        if ([pre10 evaluateWithObject:@"533252@qq.xom"])
        {
            NSLog(@"邮箱格式正确...");
        }
        else{
